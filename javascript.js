...
function setTheme(theme, isInitialLoad = false) {
    ...
    if (theme === 'emerald') {
        ...
        cssRules = `
            #app .bg-slate-900, #app .bg-slate-800, #app .bg-slate-950, #sidebarMenu { background-color: #064e3b !important; }
            #app .border-slate-700, #app .border-slate-800, #sidebarMenu, #sidebarMenu .border-slate-800 { border-color: #047857 !important; }
        `;
        ...
        if(bannerContainer) bannerContainer.innerHTML = '';
        cssRules = `
            #app .bg-slate-900, #app .bg-slate-800, #app .bg-slate-950, #sidebarMenu { background-color: #3b0764 !important; }
            #app .border-slate-700, #app .border-slate-800, #sidebarMenu, #sidebarMenu .border-slate-800 { border-color: #7e22ce !important; }
        `;
        ...
        cssRules = `
            #app .bg-slate-900, #app .bg-slate-800, #app .bg-slate-950, #sidebarMenu { background-color: #713f12 !important; }
            #app .border-slate-700, #app .border-slate-800, #sidebarMenu, #sidebarMenu .border-slate-800 { border-color: #ca8a04 !important; }
        `;
    } else if (theme === 'light') {
        ...
        // Memaksa background tampil secara inline mutlak
        body.style.setProperty('background-image', "url('background.svg')", 'important');
...
function goBackOrHome(e) {
    ...
    if (activeHeader.includes("PNIEL Oebobo")) {
        ...
        // Cek apakah ada riwayat navigasi internal PWA
        if (history.state && history.state.func) {
            history.back();
        } else {
            // Jika kosong (khususnya di PC/Laptop), kembalikan paksa ke beranda
            switchTab('home', true);
        }
    }
}
        ...
        setTimeout(() => {
            ...
            tooltip.innerHTML = `
                <div class="flex justify-between items-start mb-2">
                    <span class="bg-purple-900/60 text-purple-300 text-[10px] px-2 py-0.5 rounded font-bold">Langkah ${index + 1} dari ${steps.length}</span>
                </div>
                ...
                <div class="flex justify-end gap-2">
                    <button id="ftueSkip" class="px-3 py-1.5 text-xs text-slate-400 hover:text-slate-200 transition">Lewati</button>
                    <button id="ftueNext" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-1.5 rounded-xl text-xs font-bold transition shadow-md">${isLast ? 'Selesai' : 'Lanjut ❯'}</button>
                </div>
            `;
